...
final class WishMakerViewController: UIViewController {
    ...
    private let redSlider = CustomSlider(title: "Red", min: Constants.sliderMin, max: Constants.sliderMaxRed)
    private let blueSlider = CustomSlider(title: "Blue", min: Constants.sliderMin, max: Constants.sliderMaxRGB)
    private let greenSlider = CustomSlider(title: "Green", min: Constants.sliderMin, max: Constants.sliderMaxRGB)
    ...
    private func configureUI() {
        ...
        applySliderLabels(RemoteSliderLabelService.shared.currentLabels())
        syncSliderLabels()
    }
    ...
    private func configureSliders() {
        ...
        redSlider.valueChanged = { [weak self] (value: Double) in
            self?.redValue = CGFloat(value)
            self?.updateBackgroundColor()
        }

        greenSlider.valueChanged = { [weak self] (value: Double) in
            self?.greenValue = CGFloat(value) / 255.0
            self?.updateBackgroundColor()
        }

        blueSlider.valueChanged = { [weak self] (value: Double) in
    ...
    private func applySliderLabels(_ labels: SliderLabels) {
        redSlider.setTitle(labels.red)
        greenSlider.setTitle(labels.green)
        blueSlider.setTitle(labels.blue)
    }

    private func syncSliderLabels() {
        Task { @MainActor in
            do {
                let labels = try await RemoteSliderLabelService.shared.syncLabels()
                applySliderLabels(labels)
            } catch {
                applySliderLabels(RemoteSliderLabelService.shared.currentLabels())
            }
        }
